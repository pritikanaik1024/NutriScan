...
# Alternatively, you can fill missing values with a default value
...
df['Recipe_name'] = df['Recipe_name'].astype('category').cat.codes
df['Cuisine_type'] = df['Cuisine_type'].astype('category').cat.codes
...
df.to_csv('preprocessed_diet_data.csv', index=False)
...
def interactive_system():
    ...
    print(f"Available diet types: {', '.join(dieto_ai.diet_type_mapping.keys())}")
