import numpy as np
import pandas as pd
df = pd.read_csv('food_details.csv', delimiter=',',nrows=1, on_bad_lines='skip')
actual_columns = [col.strip() for col in df_sample.columns.tolist()]
print("Actual Columns:", actual_columns)
# handling missing values
df.fillna('', inplace=True)

# Example of ensuring consistent formatting (e.g., removing leading/trailing spaces)
...
if chunks:
    ...
    df.to_csv('Preprocessed_Nutritional_Dataset.csv', index=False)
else:
